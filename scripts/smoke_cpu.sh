...
python3 -m ntp.datagen --out data/smoke --train-tasks 60 --val-tasks 12 --eval-tasks 6 --rounds 4 --seed 0
python3 -m ntp.train --backend scratch --data data/smoke --out runs/smoke \
    ...
    --device cpu --log-every 10 --val-every 30 --warmup 5
python3 -m ntp.infer --ckpt runs/smoke/best.pt --tasks data/smoke/eval_tasks.json \
    --out runs/smoke/rollouts.json --limit 2 --device cpu
python3 -m ntp.evaluate --tasks data/smoke/eval_tasks.json --rollouts runs/smoke/rollouts.json
echo
echo "--- oracle (harness sanity: all fidelity metrics must be perfect) ---"
python3 -m ntp.evaluate --tasks data/smoke/eval_tasks.json --oracle
