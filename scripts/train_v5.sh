#!/usr/bin/env bash
# v5 recipe — CUDA box. Targets v4's diagnosis: updates are reliably downhill
# (descent 0.91) but magnitude-regressed to the small-update mode of the data
# (progress 0.11). Three magnitude-targeted levers:
#   1. --delta-token-weight 3: the <DELTA> digits get 3x loss weight (the long trace
#      otherwise soaks up most of the gradient)
#   2. --dup-early 3 + back to 6-round data: big-update rounds dominate the target
#      distribution instead of tiny late-round updates
#   3. DAgger corrections upsampled 3x in phase 2 (pass the file to --extra-train
#      three times)
# Includes the pre-DAgger eval so delta-effect vs dagger-effect is decomposed.
# Capacity variant: MODEL=Qwen/Qwen2.5-1.5B LORA=1 GRADCKPT=1 NAME=v5_qwen15 BS=4 ACCUM=2
set -euo pipefail
cd "$(dirname "$0")/.."
...
[ -f "$DATA/train.jsonl" ] || python3 -m ntp.datagen --out "$DATA" \
    --train-tasks 8000 --val-tasks 200 --eval-tasks 50 --rounds 6 --seed 5 \
    --delta --jitter-frac 0.6 --jitter-sigma 0.05,0.15,0.30 --dup-early 3

# phase 1: base training with magnitude-weighted loss
